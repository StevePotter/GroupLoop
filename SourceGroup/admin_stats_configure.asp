...
if Request("Submit") = "Update" then
	Query = "SELECT IncludeStats, IncludeMemberStats, StatTopMax, StatsTitle, MemberStatsTitle FROM Configuration WHERE CustomerID = " & CustomerID
	Set rsConfig = Server.CreateObject("ADODB.Recordset")
	rsConfig.Open Query, Connect, adOpenStatic, adLockOptimistic

	rsConfig("IncludeStats") = GetCheckedResult(Request("IncludeStats"))
	rsConfig("IncludeMemberStats") = GetCheckedResult(Request("IncludeMemberStats"))
	...
	if Request("MemberStatsTitle") <> "" then rsConfig("MemberStatsTitle") = Format( Request("MemberStatsTitle") )
...
%>
		<tr>
			...
			<td class="<% PrintTDMain %>" align="left">
				<%	PrintCheckBox IncludeStats, "IncludeStats" %> The entire site<br>
				<%	PrintCheckBox IncludeMemberStats, "IncludeMemberStats" %> Each individual member<br>
			</td>
		</tr>
		<tr>
			<td class="<% PrintTDMain %>" valign="middle" align="right">
				Section label for main site statistics
			</td>
			<td class="<% PrintTDMain %>" align="left">
		...
		<tr>
			<td class="<% PrintTDMain %>" valign="middle" align="right">
				Section label for individual member statistics.  This is displayed right after their name.  For example, if I click on 
				Joe's name, I get a page with Joe's information, including his statstics.  Right above his statistics it says <i>Joe's <%=MemberStatsTitle%></i>.  
				If you set this to "Additions to the Site", "Joe's Additions to the Site" will be displayed when someone clicks on his name.
			</td>
			<td class="<% PrintTDMain %>" align="left">
				<input type="text" name="MemberStatsTitle" value="<%=MemberStatsTitle%>" size="30">
			</td>
		</tr>
		<tr>
			<td class="<% PrintTDMain %>" valign="middle" align="right">
