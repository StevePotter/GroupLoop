...
if not LoggedMember then Redirect("members.asp?Source=members_announcements_add.asp")
...
<p align="<%=HeadingAlignment%>"><span class=Heading>Add An Announcement</span><br>
...
if Request("Submit") = "Add" then
	...
	if SendEMail then
		...
		Mailer.FromAddress = MailerFromAddress
	end if
...
'------------------------End Code-----------------------------
%>
	<p>Your announcement has been added. &nbsp;<a href="members_announcements_add.asp">Click here</a> to add another.<br>
<%
	if intTargetID <> "" then
%>
	<a href="announcements_read.asp?ID=<%=intTargetID%>">Click here</a> to read it.
<%
	end if
%>
	</p>
...
%>
	...
	* indicates required information<br>
	<form method="post" action="<%=SecurePath%>members_announcements_add.asp" name="MyForm" onsubmit="<%=GetOnAESubmit()%>if (this.submitted) return false; this.submitted = submit_page(this); return this.submitted">
...
%>
		...
		<tr> 
			<td class="<% PrintTDMain %>" align="right">Should this announcement be sent via e-mail to all the site members?</td>
			<td class="<% PrintTDMain %>"> 
...
%>
		<tr> 
			<td class="<% PrintTDMain %>" align="right">Should this announcement be added to the <%=NewsTitle%> as well?</td>
			<td class="<% PrintTDMain %>"> 
...
Sub AddAnnouncement( intCustID )
	intCustID = CInt(intCustID)

	Query = "SELECT ID, Private, MemberID, Subject, Body, CustomerID, IP, ModifiedID FROM Announcements"
...
'		rsNew.Filter = ""
		rsNew.Close
		Query = "SELECT ID, FirstName, LastName, EMail1 FROM Members WHERE CustomerID = " & intCustID & " AND EMail1 <> '' AND SubscribeAnnouncements = 1"
		rsNew.Open Query, Connect, adOpenForwardOnly, adLockReadOnly, adCmdTableDirect
		...
		strTSubject = Title & " - announcement by " & strAuthor & " - " & strSubject

		strBody = "This announcement was automatically sent to you by " & strAuthor & "'s request.  Please do not respond to this e-mail.  " & _
		...
		'not active, made problems!!! strBody = Replace(strBody, "inserts", "http://www.GroupLoop.com/" & SubDirectory & "/inserts")
