...
<p align="<%=HeadingAlignment%>"><span class=Heading>Modify Invoice Charges</span><br>
...
if not LoggedStaff() then Redirect("login.asp?Source=invoicecharges_modify.asp&ID=" & Request("ID"))

strSubmit = Request("Submit")

if strSubmit = "Update" then
	if Request("ID") = "" or Request("InvoiceID") = "" then Redirect("error.asp?Message=" & Server.URLEncode("You are missing the ID."))
	...
	With cmdReviews
		...
		.CommandText = "UpdateCustomerInvoiceCharge"
		...
		.Parameters("@Date") = AssembleDate("Date")
	...
	'if we swtiched to a new invoice, update the old price
	intOldInvoice = CInt(Request("OldInvoiceID"))
	if intOldInvoice <> intInvoiceID then currTemp = UpdateInvoicePrice( intOldInvoice )



'------------------------End Code-----------------------------
%>
<p>The invoice charge has been edited.<br>
<a href="invoices_modify.asp?Submit=Edit&ID=<%=intInvoiceID%>">View the invoice the charge belongs to.<br>
...
elseif strSubmit = "Delete" then
	...
	Query = "SELECT ID, InvoiceID, CustomerID FROM CustomerInvoiceCharges WHERE ID = " & intID
	...
	intInvoiceID = rsUpdate("InvoiceID")
...
'------------------------End Code-----------------------------
%>
	<p>The invoice charge has been deleted.  <a href="invoices_modify.asp?Submit=Edit&ID=<%=intInvoiceID%>">View the invoice</a><br>
...
elseif strSubmit = "Edit" then
	...
	Query = "SELECT ID, InvoiceID, Date, Total, Description, Hours, CustomerNote, StaffNote, DateStarted, DateEnded FROM CustomerInvoiceCharges WHERE ID = " & intID
	Set rsEdit = Server.CreateObject("ADODB.Recordset")
	rsEdit.Open Query, Connect, adOpenStatic, adLockReadOnly

	intInvoiceID = rsEdit("InvoiceID")
...
%>
	...
	* indicates required information<br>
	<form method="post" action="invoicecharges_modify.asp" name="MyForm" onsubmit="if (this.submitted) return false; this.submitted = submit_page(this); return this.submitted">
	<input type="hidden" name="ID" value="<%=rsEdit("ID")%>">
	<input type="hidden" name="OldInvoiceID" value="<%=intInvoiceID%>">
...
%>
	...
	<tr> 
		...
		<td class="<% PrintTDMain %>"> 
			<% PrintInvoicePullDown intInvoiceID %>
		</td>
	</tr>
	<tr> 
...
'-----------------------Begin Code----------------------------
	...
	Query = "SELECT ID, Date, Name FROM CustomerInvoiceCharges ORDER BY CustomerID, ID"
...
'-----------------------End Code----------------------------
%>
		<form METHOD="POST" ACTION="invoicecharges_modify.asp">
<%
'-----------------------Begin Code----------------------------
		PrintPagesHeader
		PrintTableHeader 0
...
%>
				...
				<input type="hidden" name="ID" value="<%=ID%>">
					<tr>
						...
						<input type="button" value="Delete" onClick="DeleteBox('If you delete this topic, there is no way to get it or its messages back.  Are you sure?', 'invoicecharges_modify.asp?Submit=Delete&ID=<%=ID%>')"></td>
					</tr>
				</form>
<%
'-----------------------Begin Code----------------------------
...
'------------------------End Code-----------------------------
%>
		<p>You have to create invoice charges for you can modify them, <%=GetNickNameSession()%>.</p>
<%
'-----------------------Begin Code----------------------------
	end if
